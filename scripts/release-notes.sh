...
# is a Conventional Commit subject on every commit, so the grouping is derived from
...
printf '%s\n' "$sections" | while IFS=: read -r type heading; do
  ...
  printf '%s\n' "$lines" | while IFS=$'\t' read -r subject hash; do
    # Drop the type prefix — the heading already said it, and "feat(engine): x"
    # reads better as "engine: x".
    text=${subject#*: }
...
repo=${GITHUB_REPOSITORY:-LeVanAnhDuc/web-game-match-3}
if [ -n "$previous" ]; then
